...
public class ChenGenParallel : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (update_flag != 0)
        {
            ...
            if (useJobs)
            {
                NativeArray<int> Rx_MPC1_Parallel = new NativeArray<int>(Rx_MPC1.Count, Allocator.TempJob);
                NativeArray<int> Tx_MPC1_Parallel = new NativeArray<int>(Tx_MPC1.Count, Allocator.TempJob);
                NativeArray<int> Output_Parallel = new NativeArray<int>(Rx_MPC1.Count, Allocator.TempJob);

                for (int i = 0; i < Rx_MPC1.Count; i++)
                {
                    Rx_MPC1_Parallel[i] = Rx_MPC1[i];
                    Output_Parallel[i] = 0;
                }
                for (int i = 0; i < Tx_MPC1.Count; i++)
                {
                    Tx_MPC1_Parallel[i] = Tx_MPC1[i];
                }

                // create job instance
                CommonMPC1Parallel commonMPC1Parallel = new CommonMPC1Parallel
                {
                    Array1 = Rx_MPC1_Parallel,
                    Array2 = Tx_MPC1_Parallel,
                    Output = Output_Parallel
                };

                // job scheduling
                JobHandle jobHandle = commonMPC1Parallel.Schedule(Rx_MPC1.Count, 100);
                jobHandle.Complete();// tell job to complete

                common_mpc1 = new List<int>();
                for (int i = 0; i < Output_Parallel.Length; i++)
                {
                    if (Output_Parallel[i] == 1)
                    {
                        common_mpc1.Add(Rx_MPC1_Parallel[i]);
                    }
                }

                Rx_MPC1_Parallel.Dispose();
                Tx_MPC1_Parallel.Dispose();
                Output_Parallel.Dispose();
            }
            else
            {
